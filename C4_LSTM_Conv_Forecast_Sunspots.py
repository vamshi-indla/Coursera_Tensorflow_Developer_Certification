...
# YOUR CODE HERE. READ TEMPERATURES INTO TEMPS
    ...
    for row in csvfile.readlines():
      temps.append(np.float(row.split(",")[1]))
      cntr += 1
      time_step.append(cntr)
...
import pandas as pd
df  = pd.DataFrame({'lr':history.history["lr"], 'loss': history.history["loss"]})
df.sort_values('loss', ascending=True).head(1)
...
#useful only when there is validatin data
